...
/*
proc sort data=tag99;
by final_barcode;
run;

proc contents data=w.tag99; run;

data analysis_ready;
merge tag99 weights_for_merge;
by final_barcode;
run;*/

/* merging tag99 variables */

/*data final_weight_for_merge;
set w.base_weights_calibration_weight ;
keep final_barcode final_weight gender new_age_cat;
run;

proc sort data=final_weight_for_merge;
by final_barcode;
run;

proc sort data=w.tag99;
by final_barcode;
run;

data w.data_for_analysis;
merge w.tag99 final_weight_for_merge;
by final_barcode;
if final_barcode = "300063" then delete;
run;*/

/* looking at change in design effect and mean with trimmed vs untrimmed weights */

data trimmed99;
set initial_analysis;
if final_weight > 2099.8667 then final_weight = 2099.8667;
run;

data trimmed95;
set initial_analysis;
if final_weight > 1258.3155 then final_weight = 1258.3155;
run;
